	...
	function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY A.DISTRIK_ID ASC")
	{
		$str = "
			SELECT	A.*,
				...
				, B.NAMA AREA_NAMA
				, B.KODE AREA_KODE
				, B.DESKRIPSI AREA_DESKRIPSI
				, C.NAMA DISTRIK_NAMA
				, CASE WHEN A.TERSEDIA = 1 THEN 'Iya' WHEN A.TERSEDIA = 2 THEN 'Tidak' ELSE 'Belum Didefinisikan' END TERSEDIA_INFO
				, CASE WHEN A.KODE_DUPLIKAT IS NOT NULL 
				  THEN B.KODE || GENERATEZERO(D.KODE,1)  || ' - '||  B.NAMA 
				  ELSE
				  A.KODE_DUPLIKAT
				  END KODE_INFO
			...
			INNER JOIN ITEM_ASSESSMENT_DUPLIKAT D ON D.KODE= A.KODE_DUPLIKAT AND D.LIST_AREA_ID= A.LIST_AREA_ID
