...
class outlining_assessment_json extends CI_Controller
{
		...
		if($reqSimpan == 1 )
		{
			if(!empty($reqListAreaId))
			{
				...
				foreach ($reqListAreaId as $key => $value) {
					...
					$setdetil->setField("AREA_UNIT_DETIL_ID", $reqAreaUnitDetilId[$key]);
	...
	function filter_unit()
	{
		...
		$statement .=" AND A.STATUS IS NULL AND A.NAMA IS NOT NULL AND EXISTS(SELECT A.UNIT_MESIN_ID FROM AREA_UNIT B WHERE A.UNIT_MESIN_ID=B.UNIT_MESIN_ID)";
		$set->selectByParams(array(), -1,-1,$statement);
		// echo $set->query;exit;
		while($set->nextRow())
		{
