...
class tindak_lanjut_json extends CI_Controller
{
	...
	function json()
	{
		$this->load->model("base-app/TindakLanjut");
		...
		{
			$statement .= " AND DR.DISTRIK_ID ='".$reqDistrikId."'";
		}
		else
		{
			...
			else
			{
				...
				if(!empty($idDistrik))
				{
					$statement .=" AND DR.DISTRIK_ID IN (".$idDistrik.")";
				}
			}  
		...
		if(!empty($reqBlok))
		{
			$statement .= " AND BU.BLOK_UNIT_ID ='".$reqBlok."'";
		}
		...
		if(!empty($reqNotif))
		{
			...
			{
				$statement .= " AND A.OUTLINING_ASSESSMENT_REKOMENDASI_ID IS NULL ";
			}
			...
			{
				$statement .= " AND A.OUTLINING_ASSESSMENT_REKOMENDASI_ID IS NOT NULL  AND A.KETERANGAN ='' ";
			}
		...
		else
		{
			...
			// else
			// {
			// 	$statement .= " AND E.OUTLINING_ASSESSMENT_REKOMENDASI_ID IS NOT NULL ";
			// }
		}

		
		$set= new TindakLanjut();

		$sOrder = " ORDER BY C.TAHUN,C.BULAN,C.DISTRIK_ID ASC ";
		$set->selectByParamsRekomendasi(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);
