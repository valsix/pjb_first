...
class konfirmasi_item_assessment_json extends CI_Controller
{
	...
	function add()
	{
		...
		if ($reqMode == "insert")
		{
			...
			$statement=" AND A.NILAI =  '".$reqNilai."' ";
			$check = new KonfirmasiItemAssessment();
			$check->selectByParams(array(), -1, -1, $statement);
			// echo $check->query;exit;
			$check->firstRow();
			$checkNilai= $check->getField("NILAI");


			if(!empty($checkNilai))
			{
				echo "xxx***Nilai ".$checkNilai." sudah ada";exit;	
			}
			unset($check);
		...
		{	
			$statement=" AND A.KONFIRMASI_ITEM_ASSESSMENT_ID =  '".$reqId."' ";
			$check = new KonfirmasiItemAssessment();
			$check->selectByParams(array(), -1, -1, $statement);
			// echo $check->query;exit;
			$check->firstRow();
