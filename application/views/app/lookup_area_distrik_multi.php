        ...
        <div class="row row-treegrid">
            ...
            <div class="col-md-9 col-treegrid">
                <div class="area-konten-atas">
                    <div class="judul-halaman">Area </div>
                ...
                <div id="tableContainer" class="tableContainer tableContainer-treegrid">
                    <table id="treeSatker" class="easyui-treegrid" style="min-width:100px !important;height:300px"
                            data-options="
                                url: 'json-app/lookup_area_distrik_json/multi?reqId=<?=$reqId?>',
                                pagination: true, 
                                method: 'get',
                                idField: 'id',
                                treeField: 'text',
                        ...
                        <thead>
                            <tr>
                                <th data-options="field:'id',width:100" formatter="formatcheckbox">Area</th>
                                <th data-options="field:'AREA_UNIT',width:100" >Area</th>
                            </tr>
                        </thead>
                    </table>
                </div>
...
function setsatuankerjapilih()
{
    ...
    else
    {
        ...
        infoid.forEach(function (item, index) {
            ...
            vitemdplid.push(String(arritem[1]));
        });
        ...
        // console.log(infoid);
        var divIdField = "outlinedetil";
        top.addmultiarea(vkelompokequipmentid,vitemdplid, infonama,infoidareaunit,infoidareaunitdetil, divIdField);
        top.closePopup();
    }
}

// function show(checkid,ujiid)
function show(checkid,checkiddpl)
{

    // var s = '#check_'+checkid+'_'+ujiid+'';
    var checkid=checkid+'_'+checkiddpl;
    ...
    idselected= nodes.id;
    iddplselected= nodes.ITEM_ASSESSMENT_DUPLIKAT_ID;
    idareaunitselected= nodes.AREA_UNIT_ID;
    idareaunitdetilselected= nodes.AREA_UNIT_DETIL_ID;
    
    nama= nodes.NAMA;
    unit= nodes.AREA_UNIT;
    var tujuan= nama+' - '+unit;
        ...
        if(infochecked == false)
        {
            ...
            if(parseInt(elementRow) >= 0)
            {
                ...
                infoidareaunit.splice(elementRow, 1);
                infoidareaunitdetil.splice(elementRow, 1);
            }
        }
        else
        {
            ...
            infoidareaunit.push(String(idareaunitselected));
            infoidareaunitdetil.push(String(idareaunitdetilselected));
        }
        // setinfo(ujiid);
...
{
    idata=0;
    infodetilparaf= '<label>Area Terpilih:</label><button class="btn btn-primary btn-sm" type="button" onClick="setsatuankerjapilih()"><i class="fa fa-user-circle"></i> Ok</button>';
    ...
    if(idata == 0)
    {
        infodetilparaf= '<label>Area Terpilih: Pilih salah satu data</label><button class="btn btn-primary btn-sm" type="button" onClick="setsatuankerjapilih()"><i class="fa fa-user-circle"></i> Ok</button>';
    }
...
function formatcheckbox(val,row)
{
    // return "<input type='checkbox' onclick=show('"+row.LIST_AREA_ID+'_'+row.KODE_DUPLIKAT+"') id='check_"+row.LIST_AREA_ID+'_'+row.KODE_DUPLIKAT+"' "+(row.checked?'checked':'')+"/> " + row.text;
     return "<input type='checkbox' onclick=show('"+row.LIST_AREA_ID+"','"+row.ITEM_ASSESSMENT_DUPLIKAT_ID+"') id='check_"+row.LIST_AREA_ID+"_"+row.ITEM_ASSESSMENT_DUPLIKAT_ID+"' "+(row.checked?'checked':'')+"/> " + row.text;
...
$(document).ready( function () {     
    ...
    $('input[name=reqPencarian]').keyup(function(e) {
        ...
        if(e.keyCode == 13) {
            var urlApp = 'json-app/lookup_area_distrik_json/multi/?reqId=<?=$reqId?>&reqPencarian='+value;
            $('#treeSatker').treegrid(
            {
                url: urlApp,
