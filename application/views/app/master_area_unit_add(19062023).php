...
else
{
    ...
    // print_r($set->getField("LIST_AREA_ID_INFO"));exit;
...
var tipe = $("#reqListAreaId");
    tipe.on("select2:select", function(event) {
      ...
      jQuery.each(values, function(index, value) {
        $.get("app/loadUrl/app/master_area_unit_add_detil?reqListAreaId="+values+"&reqId=<?=$reqId?>", function(data) {
            $('#tabelareaunit').empty();   
