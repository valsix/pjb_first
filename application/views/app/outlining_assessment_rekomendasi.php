...
{
    $reqAreaDetilId=$set->getField("OUTLINING_ASSESSMENT_AREA_DETIL_ID");
}
if(empty($reqId))
{
...
if(empty($reqDetilId))
{
    $reqDetilId=$set->getField("OUTLINING_ASSESSMENT_DETIL_ID");
}
...
$set= new OutliningAssessment();
$set->selectByParamsRekomendasiMaxTahun(array(), -1,-1);
// echo $set->query;exit;
$set->firstRow();
$reqMaxTahun=$set->getField("MaxTahun");
...
$set= new Approval();
$set->selectalllistapproval("outlining_assessment_rekomendasi");
$set->firstRow();
$reqCheckUserRoleid= $set->getField("ROLE_ID");
unset($set);

$reqCheckTombol="";
if($appuserroleid==$reqCheckUserRoleid)
{
    $reqCheckTombol=1;
}
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select name="reqBulan" id="reqBulan" class="form-control datatable-input">
                                        ...
                                        {
                                            ?>
                                            <option value="<?=$arrBulan[$i]?>"><?=getNameMonthNew($arrBulan[$i])?></option>
                                            <?    
                                        }
                                        ?>
                                    </select>
...
<!-- 
                        ...
                        <div style="overflow-y: auto;height: 100%;width: 100%;">
                            <?
                            if(!empty($reqCheckTombol))
                            {
                            ?>
                                <div style="text-align:right;padding:5px">
                                    <a href="javascript:void(0)" class="btn btn-primary" id='approveall'>Approve All</a>
                                </div>
                            <?
                            }
                            ?>
                            <table id="" class="table table-bordered table-striped table-hovered" style="width: 100%;" >
                                <thead >
                                    <tr>
                                        ...
                                        <th class="text-center" style="vertical-align: middle">Distrik</th>
                                        <th class="text-center" style="vertical-align: middle">Blok Unit</th>
                                        <th class="text-center" style="vertical-align: middle;width: 20%">Status</th>
                                        <th class="text-center" style="vertical-align: middle;width: 20%">Area Terisi/Total Area</th>
                                        <th class="text-center" style="vertical-align: middle;width: 10%">% Penyelesaian</th>
                                    </tr>
                                </thead>
                                <tbody id="distrikm" >
                                </tbody>
                    ...
                    else  if($reqPage==2)
                    {
                        ...
                        $statement=" AND B.DISTRIK_ID=".$reqDistrikId." AND A.BLOK_UNIT_ID=".$reqBlokId." AND A.OUTLINING_ASSESSMENT_ID=".$reqId;
                        if(!empty($reqDetilId))
                        {
                            $statement.=" AND A.OUTLINING_ASSESSMENT_DETIL_ID=".$reqDetilId;
                        }
                                ...
                                <table id="page2" class="table table-bordered table-striped table-hovered" style="width: 100%;" >
                                    <thead >
                                        <tr>
                                            ...
                                            <th class="text-center" style="vertical-align: middle">Terisi / Total Rekomendasi</th>
                    ...
                    else if($reqPage==4)
                    {
                        ...
                        if($reqPenggunaHakId == 3 || $reqPenggunaHakId == 2  )
                        {
                            if(!empty($reqDetailRekomendasi))
                            {
                                $readonlywo="";
            ...
            ?>
                <div style="text-align:center;padding:5px">
                    ...
                    ?>
                        ...
                        {
                            if($reqPenggunaHakId == 1 || $reqPenggunaHakId==7  || $reqPenggunaHakId==8)
                            {
                        ...
                        else
                        {
                            if($reqVstatus==20)
                            {
                                if($reqPenggunaHakId == 2 || $reqPenggunaHakId==3  )
                                {
...
function approveall()
{
    var ref_tabel="outlining_assessment_rekomendasi";
    // console.log(pg);
    $.messager.confirm('Konfirmasi',"Approve all data Rekomendasi?",function(r){
        if (r){
            $.ajax({
               type: "POST",
               data: {ref_tabel:ref_tabel},
               url: "json-app/Approval_multi_json/approve",
...
$('#reqBulan,#reqTahun,#reqDistrik,#reqBlok,#reqStatus').on('change', function() {
    ...
    // console.log(bulan);
    $("#distrikm").empty();
    $.getJSON("json-app/outlining_assessment_json/filter_rekomendasi?reqTahun="+tahun+"&reqDistrik="+reqDistrik+"&reqBlok="+reqBlok+"&reqStatus="+reqStatus+"&reqVstatus=<?=$reqVstatus?>",
    function(data){
        data.forEach(function(entry) {
        // console.log();
         $('#distrikm').append("<tr class='distrik'><td style='display: none'>"+entry.DISTRIK_ID+"</td><td style='display: none'>"+entry.BLOK_UNIT_ID+"</td><td style='display: none'>"+entry.OUTLINING_ASSESSMENT_ID+"</td><td class='text-center' style='vertical-align: middle'><label style='font-weight: normal !important;'>"+entry.NO+"</label></td><td><label style='font-weight: normal !important;'>"+entry.BULAN_NAMA+"</label></td><td><label class='text-center' style='font-weight: normal !important;'>"+entry.TAHUN+"</label></td><td><label style='font-weight: normal !important;'>"+entry.DISTRIK_NAMA+"</label></td><td><label style='font-weight: normal !important;'>"+entry.BLOK_NAMA+"</label></td><td>"+entry.STATUS+"</td><td class='text-center'>"+entry.TERISI+' / '+entry.TOTAL+"</td><td>"+entry.PERC_AREA+" %</td></tr>");
       });
    });
});
...
$(function() {
    ...
    // $('tr').dblclick(function(){
        ...
        {
            window.open('app/index/outlining_assessment_rekomendasi?reqAreaDetilId='+reqAreaDetilId+'&reqDetilId='+reqDetilId+'&reqId='+reqId+'&reqKembali=1&reqPage='+reqPage+'&reqDistrikId='+reqDistrikId+'&reqBlokId='+reqBlokId+'&reqAreaDuplikatId='+reqAreaDuplikatId+'&reqAreaId='+reqAreaId+'&reqRekomendasiId='+reqRekomendasiId, '_blank'); 
        }
...
function filterrekomendasi(bulan,tahun) {
   $("#distrikm").empty();
       $.getJSON("json-app/outlining_assessment_json/filter_rekomendasi?reqTahun="+tahun+"&reqVstatus=<?=$reqVstatus?>",
        function(data){
            data.forEach(function(entry) {
            // console.log();
...
$.getJSON("json-app/outlining_assessment_json/filter_blok?reqDistrikId="+reqDistrikId,
  ...
  $('#reqBlok').on('change', function() {
    ...
    $.getJSON("json-app/outlining_assessment_json/filter_unit?reqDistrikId="+reqDistrikId+"&reqBlokId="+reqBlokId+"&reqTahun="+reqTahun,
        function(data)
        {
            $("#reqUnitMesin option").remove();
