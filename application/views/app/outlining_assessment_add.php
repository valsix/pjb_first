...
else
{
    ...
    $reqArrBlokId= getmultiseparator($set->getField("BLOK_UNIT_ID_INFO"));
    $reqBlokId= $set->getField("BLOK_UNIT_ID_INFO");
...
else
{
    ...
    AND A.BLOK_UNIT_ID IN = (".$reqBlokId.")";
}
        ...
        <div class="konten-inner">
            ...
            <div>
                ...
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data" autocomplete="off">
                    ...
                    <input type="hidden" name="infopgnew" value="outlining_assessment_detil" />
                    <?
                    ...
                    if($vstatus==20)
                    {   
                        $approval_table= "outlining_assessment_detil";
                        $approval_field_id= "OUTLINING_ASSESSMENT_ID";
                        $approval_field_status= "V_STATUS_1";
                        $vappr= new libapproval();
                        $arrparam= ["approval_info_pg"=>$approval_table, "approval_info_id"=>$reqId, "approval_table"=>$approval_table, "approval_field_id"=>$approval_field_id, "approval_field_status"=>$approval_field_status];
                                // print_r($arrparam);
                        $vappr->viewnew($arrparam);
                    }
                    ?>
                    ...
                    <div class="form-group">  
                        ...
                        <div class='col-md-8'>
                            <div class='form-group'>
                                ...
                                <select class="form-control jscaribasicmultiple"  id="reqBlokId" <?=$readonlyblok?>  <?=$disabled?> name="reqBlokId[]"  style="width:100%;" multiple >
                                <?
                                foreach($arrblok as $item) 
                                {
                                    ...
                                    //     $selected="selected";
                                    // }
                                    if( in_array($selectvalid, $reqArrBlokId))
                                    {
                                        $selected="selected";
                                    }
                             ...
                             <table id="tabel" class="table table-bordered" style="width: 100%;" >
                                ...
                                <tbody id="outlinedetil">
                                    ...
                                    ?>
                                        ...
                                        ?>
                                            <tr id="<?=$reqUniqId?>" class="detilcoba">
                                                <td style="display: none"><input class="easyui-validatebox textbox form-control" type="hidden" name="reqListAreaId[]" id="reqListAreaId<?=$reqUniqId?>"   value="<?=$reqListAreaId?>"   ><?=$reqListAreaId?></td>
                                                <td style="display: none"> <input class="easyui-validatebox textbox form-control" type="hidden" name="reqItemAssessmentDuplikatId[]" id="reqItemAssessmentDuplikatId<?=$reqUniqId?>"   value="<?=$reqItemAssessmentDuplikatId?>"  ><?=$reqItemAssessmentDuplikatId?></td>
                                                <td style="display: none"> <input class="easyui-validatebox textbox form-control" type="hidden" name="reqAreaUnitDetilId[]" id="reqAreaUnitDetilId<?=$reqUniqId?>"   value="<?=$reqAreaUnitDetilId?>"  ><?=$reqAreaUnitDetilId?></td>
                                                <td style="display: none"> <input class="easyui-validatebox textbox form-control" type="hidden" name="reqAreaUnitId[]" id="reqAreaUnitId<?=$reqUniqId?>"   value="<?=$reqAreaUnitId?>"  ><?=$reqAreaUnitId?></td>
                                                <td style="display: none"> <input class="easyui-validatebox textbox form-control" type="hidden" name="reqBlokUnitId[]" id="reqBlokUnitId<?=$reqUniqId?>"   value="<?=$reqBlokUnitId?>"  ><?=$reqBlokUnitId?></td>
                                               
                                                <td style="vertical-align: middle;text-align: center" class="sno"> </td>
                                                <td style="vertical-align: middle;"> <?=$reqNamaBlok?></td>
                                                <td style="vertical-align: middle;"> <?=$reqNamaMesin?></td>
                                                ...
                                                <td style="text-align: center;vertical-align: middle;" > <?=$jumlahstatusconfirm?></td> 
                                                <td style="text-align: center;vertical-align: middle;"> <?=$jumlahstatusnotconfirm?></td>
                                                <td style="text-align: center;vertical-align: middle;"><span  class='hapustabel' style='background-color: red; padding: 10px; border-radius: 5px;top: 50%;position: relative;'><a onclick='HapusDetil("<?=$reqDetilId?>","<?=$reqId?>")'><i class='fa fa-trash fa-lg' style='color: white;' aria-hidden='true'></i></a></span>
                                                </td>  
            ...
            ?>
                <div style="text-align:center;padding:5px">
                    ...
                    {
                    ?>
                        <input type='hidden' name='is_draft' id="is_draft" value='<?=$vstatus?>'>
                    <?
                    }
                    ?>
                    <?
                    ...
                    <input type='hidden' name='is_draft1' id="is_draft1" value='<?php echo ($vstatus1==1)?1:0?>'>
                    ...
                    ?>
                        ...
                        <a href="javascript:void(0)" class="btn btn-warning" id='update1' >Kirim Hasil</a>
...
if(cekroleid == 1 || cekroleid == 3)
{
    ...
    else
    {
        if(check_vstatus==20)
        {
            if(check_vstatus1==1 || check_vstatus1 =='' || check_vstatus1 ==90)
            {
                $('#update1').show();
            }
        }
...
if(check_vstatus !=20 && checkkirim==1 )
{
    $('#simpannew').show();
...
if(check_vstatus !=20 )
{
     if(check_vstatus==1 || check_vstatus==10)
     {
        if(checkkirim==1 && check_vstatus==10 )
        {
         $('.hapustabel').show();
        }
        if(check_vstatus==1)
     ...
     {
        if(checkkirim==1 && check_vstatus==0 )
        {
         $('.hapustabel').show();
        }
        else
...
$("#outlinedetil").on("click", ".btn-remove", function(){
...
$('#outlinedetil tr').dblclick(function(){
  ...
  var readonly="";
    ...
    else
    {
        ...
        window.open('app/index/outlining_assessment_add_detil?reqId=<?=$reqId?>&reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>&reqDetilId='+reqDetilId+'&reqListAreaId='+reqListAreaId+'&reqItemAssessmentDuplikatId='+reqItemAssessmentDuplikatId+'&reqAreaUnitDetilId='+reqAreaUnitDetilId+'&reqAreaUnitId='+reqAreaUnitId, '_blank'); 
    }
...
function openArea()
{
    openAdd('app/loadUrl/app/lookup_area_distrik_multi?reqId=<?=$reqId?>&reqUniqId=<?=$reqUniqId?>&reqBlokId=<?=$reqBlokId?>');
}
...
$('#reqBlokId').on('select2:selecting', function(e) {
    // console.log(1);
    var reqDistrikId= $("#reqDistrikId").val();
    var reqBlokId= e.params.args.data.id;
    var reqUnitMesinId= $("#reqUnitMesinId").val();
    var reqBulan= '<?=$reqBulan?>';
    var reqTahun= '<?=$reqTahun?>';
    // $("#outlinedetil").empty();

    // var tableContent = localStorage.getItem('tableContent'+reqBlokId);

    // // console.log(tableContent);
    // // console.log(tableContent);
    // var lclblokid = localStorage.getItem('reqBlokId'+reqBlokId);
    
    // if (localStorage.getItem("tableContent"+reqBlokId) !== null && lclblokid==reqBlokId ) {
    //     $("#outlinedetil").empty();
    ...
    // else
    // {
    //     //  localStorage.removeItem('reqBlokId'+reqBlokId);
    //     // localStorage.removeItem('tableContent'+reqBlokId);
...
table.on('click', 'tr', function (){
        ...
        if ( $(this).hasClass('detilcoba') ) 
        {
            $(this).removeClass('pilihtabel');
        }
        else
        {
            if ( $(this).hasClass('detildistrik') ) 
            {
                $(this).removeClass('pilihtabel');
            }
            else
            {
                if ( $(this).hasClass('pilihtabel') ) 
                {
                    $(this).removeClass('pilihtabel');
                }
                else
                {
                    $(this).addClass('pilihtabel');
                }
            }
...
// function save_check(valBlokUnitId){
//     var tableContent = document.getElementById('outlinedetil').innerHTML;
   
//     var reqBlokId= $("#reqBlokId").val();
//     var reqDistrikId= $("#reqDistrikId").val();

//     localStorage.setItem('reqBlokId'+valBlokUnitId, valBlokUnitId);
//     localStorage.setItem('tableContent'+valBlokUnitId, tableContent);
   
//     localStorage.setItem('reqDistrikId', reqDistrikId);
// }


$('#reqBlokId').on("select2:unselecting", function(e){
 ...
 if(reqId=="")
 {
    $.getJSON("json-app/outlining_assessment_json/deletedetilblok/?reqBlokId="+value+"&reqId=<?=$reqId?>",
        function(data)
        {
            ids.remove();
        });
...
else
{
 ...
 // ids.remove();
 // localStorage.removeItem('reqBlokId'+value);
 // localStorage.removeItem('tableContent'+value);
...
function HapusDetil(iddetil,reqId) {
    $.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
        if (r){
            $.getJSON("json-app/outlining_assessment_json/deleteareadetiltabel?reqDetilId="+iddetil+"&reqId="+reqId,
                function(data){
                    // console.log(data);return false;
                    $.messager.alert('Info', data.PESAN, 'info');                    
                    location.reload();
...
function submitForm(){
    ...
    var listareaid=[];
    $("#inputselect").empty();
    // $('#outlinedetil input[type=hidden]').remove();
    // $('#outlinedetil input[type=hidden]').not('input[name=reqDetilId]').remove();
    // $("input[type='hidden']").removeAttr('reqBlokUnitId');
    var checkid='<?=$reqId?>';
    // if(checkid=="")
    // {
        $("#tabel tr.pilihtabel").each(function(){
            ...
            var valBlokUnitId= $(this).closest('tr').find('td:eq(4)').text();
            // console.log(valBlokUnitId);
            ...
            var BlokUnitId =" <input  type='hidden' name='reqBlokUnitId[]' id='reqBlokUnitId' value="+valBlokUnitId+" />";

            $("#inputselect").append(ListAreaId+ItemAssessmentDuplikatId+AreaUnitDetilId+AreaUnitId+BlokUnitId);
